$(document).ready(function(){
    var country = ($('body').hasClass('Ukraine')) ? 'Ukraine' : 'Russia';
	switch (country) {
		default: 				$('.header .phone').html('<a href="tel:+380501064108" class="footer-phone"><span>+380 (50)</span> 106 41 08</a>');
								$('#how-item-2 .how-item-title').html('<span>Доставка Новой Почтой</span> по Украине');
								$('.order .order-form-subtitle').html('получите скидку до 20% <br/>и не платите за доставку');
								curr = 'byn';
    ...
    $('.color-value').on('click', function(){
        if(!$(this).hasClass('active')){
			...
			convead('event', 'view_product', {
				product_id: prod_id,
				product_name: 'Air sofa',
				product_url: 'http://promo.brast.in.ua/'
			});			
            calc();
        }
